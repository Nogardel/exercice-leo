...
app.get("/api/todos", async (req, res, next) => {
  try {
    ...
    console.log(`GET /api/todos avec userId: ${userId || "non spécifié"}`);
    const todos = await tachesService.getAllTasks(userId);
    res.json(todos);
  } catch (error) {
...
// Route GET pour récupérer les tâches terminées
app.get("/api/todos/done", async (req, res, next) => {
  try {
    const userId = req.query.userId;
    console.log(`GET /api/todos/done avec userId: ${userId || "non spécifié"}`);
    const todos = await tachesService.getDoneTasks(userId);
    res.json(todos);
  } catch (error) {
    next(error); // Passer l'erreur au middleware de gestion d'erreurs
  }
...
app.post("/api/todos", async (req, res, next) => {
  try {
    ...
    console.log(
      `POST /api/todos avec userId: ${newTodo.userId || "non spécifié"}`
    );
...
// Route GET pour récupérer uniquement les tâches terminées (route déplacée en haut)
